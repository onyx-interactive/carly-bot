...
async def send_embed(ctx, embed):
    ...
    try:
        ...
        try:
            await ctx.send("Ei, parece que não posso enviar incorporações. Por favor, verifique minhas permissões :) ")
        except Forbidden:
            await ctx.author.send(
                f"Ei, parece que não posso enviar nenhuma mensagem em {ctx.channel.name} em {ctx.guild.name}\n"
                f"Você pode informar a equipe do servidor sobre esse problema? :slight_smile:", embed=embed)
