...
async def on_ready():
    ...
    guild_string = ""
    for g in bot.guilds:
        guild_string += f"{g.name} - {g.id} - Membros: {g.member_count}\n"
        member_count += g.member_count
        
    logger.info(f"Bot '{bot.user.name}' conectou-se, ativo em {len(bot.guilds)} guildas:\n{guild_string}")
...
async def on_member_join(ctx):
    embed=discord.Embed(title=f"{str(ctx.user)} | Bem-vindo(a)!", description=" ", color=0x333333)
    embed.add_field(name="🛡 Tag do Usuário", value=f"`{str(ctx.user)}` ({str(ctx.user-id)})", inline=True)
    embed.add_field(name="📛 Precisando de ajuda?", value="Caso você tenha alguma dúvida ou problema, chame a nossa equipe!", inline=True)
    embed.add_field(name="👮 Evite punições!", value="Leia as nossas regras( #rules ) para evitar ser punido no servidor!", inline=True)
    embed.set_footer(text=f"{str(ctx.guild)} • © Todos os direitos reservados.")
    await bot.get_channel(973142820762308610).send(embed=embed)
...
async def on_member_remove(ctx):
    await bot.get_channel(973142820762308610).send(f"{ctx.name} has left")
